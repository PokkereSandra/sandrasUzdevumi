...
// count of elements in the array. Create a function that doubles the integer number.
// Within the loop using php in-built function print out the double of the number value
// (using your custom function).
$array = [1, 10, 15, 12.3, "Sandra"];
function doubleInt(int $number): int
{
    return $number * 2;
}
for ($i = 0; $i < count($array); $i++) {
    if (is_integer($array[$i])) {
        echo phpdoubleInt($array[$i]) . PHP_EOL;
    }
}
